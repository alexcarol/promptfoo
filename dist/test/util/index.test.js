...
describe('renderEnvOnlyInObject', () => {
    ...
    describe('Basic rendering', () => {
        it('should render simple dot notation env vars', () => {
            process.env.TEST_ENV_VAR = 'env_value';
            expect((0, index_3.renderEnvOnlyInObject)('{{ env.TEST_ENV_VAR }}')).toBe('env_value');
        });
        it('should render bracket notation with single quotes', () => {
            process.env['VAR-WITH-DASH'] = 'dash_value';
            expect((0, index_3.renderEnvOnlyInObject)("{{ env['VAR-WITH-DASH'] }}")).toBe('dash_value');
        });
        it('should render bracket notation with double quotes', () => {
            process.env['VAR_NAME'] = 'value';
            expect((0, index_3.renderEnvOnlyInObject)('{{ env["VAR_NAME"] }}')).toBe('value');
        });
        it('should handle whitespace variations', () => {
            process.env.TEST = 'value';
            expect((0, index_3.renderEnvOnlyInObject)('{{env.TEST}}')).toBe('value');
            expect((0, index_3.renderEnvOnlyInObject)('{{  env.TEST  }}')).toBe('value');
            expect((0, index_3.renderEnvOnlyInObject)('{{ env.TEST}}')).toBe('value');
            expect((0, index_3.renderEnvOnlyInObject)('{{env.TEST }}')).toBe('value');
        });
        it('should render empty string env vars', () => {
            ...
            expect((0, index_3.renderEnvOnlyInObject)('{{ env.EMPTY_VAR }}')).toBe('');
        });
        it('should render env vars with special characters in value', () => {
            process.env.SPECIAL_CHARS = 'value with spaces & $pecial chars!';
            expect((0, index_3.renderEnvOnlyInObject)('{{ env.SPECIAL_CHARS }}')).toBe('value with spaces & $pecial chars!');
        });
    });
    describe('Filters and expressions (NEW functionality)', () => {
        it('should support default filter with fallback', () => {
            ...
            expect((0, index_3.renderEnvOnlyInObject)("{{ env.EXISTING | default('fallback') }}")).toBe('exists');
            // NEW: When env var doesn't exist but has default filter, Nunjucks renders it
            expect((0, index_3.renderEnvOnlyInObject)("{{ env.NONEXISTENT | default('fallback') }}")).toBe('fallback');
        });
        it('should support upper filter', () => {
            process.env.LOWERCASE = 'lowercase';
            expect((0, index_3.renderEnvOnlyInObject)('{{ env.LOWERCASE | upper }}')).toBe('LOWERCASE');
        });
        it('should support lower filter', () => {
            process.env.UPPERCASE = 'UPPERCASE';
            expect((0, index_3.renderEnvOnlyInObject)('{{ env.UPPERCASE | lower }}')).toBe('uppercase');
        });
        it('should support chained filters', () => {
            process.env.TEST = 'test';
            expect((0, index_3.renderEnvOnlyInObject)("{{ env.TEST | default('x') | upper }}")).toBe('TEST');
        });
        it('should support complex filter expressions', () => {
            process.env.PORT = '8080';
            expect((0, index_3.renderEnvOnlyInObject)('{{ env.PORT | int }}')).toBe('8080');
        });
        it('should handle filter with closing brace in argument', () => {
            process.env.VAR = 'value';
            // This is a tricky case: the default value contains }
            expect((0, index_3.renderEnvOnlyInObject)("{{ env.VAR | default('}') }}")).toBe('value');
        });
    });
    describe('Preservation of non-env templates', () => {
        it('should preserve vars templates', () => {
            process.env.TEST_ENV_VAR = 'env_value';
            expect((0, index_3.renderEnvOnlyInObject)('{{ env.TEST_ENV_VAR }}, {{ vars.myVar }}')).toBe('env_value, {{ vars.myVar }}');
        });
        it('should preserve prompt templates', () => {
            process.env.TEST_ENV_VAR = 'env_value';
            expect((0, index_3.renderEnvOnlyInObject)('{{ env.TEST_ENV_VAR }}, {{ prompt }}')).toBe('env_value, {{ prompt }}');
        });
        it('should preserve multiple non-env templates', () => {
            process.env.API_HOST = 'api.example.com';
            const template = 'Host: {{ env.API_HOST }}, Message: {{ vars.msg }}, Context: {{ context }}, Prompt: {{ prompt }}';
            const expected = 'Host: api.example.com, Message: {{ vars.msg }}, Context: {{ context }}, Prompt: {{ prompt }}';
            expect((0, index_3.renderEnvOnlyInObject)(template)).toBe(expected);
        });
        it('should preserve templates with filters on non-env vars', () => {
            expect((0, index_3.renderEnvOnlyInObject)("{{ vars.name | default('Guest') }}")).toBe("{{ vars.name | default('Guest') }}");
        });
    });
    describe('Undefined env vars', () => {
        it('should preserve template if env var does not exist', () => {
            expect((0, index_3.renderEnvOnlyInObject)('{{ env.NONEXISTENT }}')).toBe('{{ env.NONEXISTENT }}');
        });
        it('should preserve bracket notation if env var does not exist', () => {
            expect((0, index_3.renderEnvOnlyInObject)("{{ env['MISSING'] }}")).toBe("{{ env['MISSING'] }}");
        });
    });
    describe('Complex data structures', () => {
        it('should work with nested objects', () => {
            ...
            expect((0, index_3.renderEnvOnlyInObject)(obj)).toEqual({
                level1: {
                    level2: {
                        env1: 'value1',
                        env2: 'value2',
        ...
        it('should work with arrays', () => {
            ...
            expect((0, index_3.renderEnvOnlyInObject)(arr)).toEqual(['value1', '{{ vars.test }}', 'value2', 42]);
        });
        it('should work with mixed nested structures', () => {
            ...
            const rendered = (0, index_3.renderEnvOnlyInObject)(config);
            expect(rendered).toEqual({
                api: {
                    key: 'secret123',
                    endpoints: ['{{ env.BASE_URL }}/users', '{{ env.BASE_URL }}/posts'],
    ...
    describe('Primitive types', () => {
        it('should handle null', () => {
            expect((0, index_3.renderEnvOnlyInObject)(null)).toBeNull();
        });
        it('should handle undefined', () => {
            expect((0, index_3.renderEnvOnlyInObject)(undefined)).toBeUndefined();
        });
        it('should handle numbers', () => {
            expect((0, index_3.renderEnvOnlyInObject)(42)).toBe(42);
        });
        it('should handle booleans', () => {
            expect((0, index_3.renderEnvOnlyInObject)(true)).toBe(true);
            expect((0, index_3.renderEnvOnlyInObject)(false)).toBe(false);
        });
    });
    describe('Error handling', () => {
        it('should preserve template on Nunjucks render error', () => {
            ...
            const rendered = (0, index_3.renderEnvOnlyInObject)(template);
    ...
    describe('PROMPTFOO_DISABLE_TEMPLATING flag', () => {
        it('should return unchanged when flag is set', () => {
            process.env.PROMPTFOO_DISABLE_TEMPLATING = 'true';
            process.env.TEST_ENV_VAR = 'env_value';
            expect((0, index_3.renderEnvOnlyInObject)('{{ env.TEST_ENV_VAR }}')).toBe('{{ env.TEST_ENV_VAR }}');
        });
        it('should return unchanged objects when flag is set', () => {
            process.env.PROMPTFOO_DISABLE_TEMPLATING = 'true';
            process.env.TEST = 'value';
            const obj = { key: '{{ env.TEST }}' };
            expect((0, index_3.renderEnvOnlyInObject)(obj)).toEqual({ key: '{{ env.TEST }}' });
        });
    });
    describe('Real-world scenarios', () => {
        it('should handle Azure provider config with mixed templates', () => {
            ...
            expect((0, index_3.renderEnvOnlyInObject)(config)).toEqual({
    ...
    describe('Edge cases', () => {
        it('should handle multiple env vars in same string', () => {
            ...
            process.env.PORT = '8080';
            expect((0, index_3.renderEnvOnlyInObject)('{{ env.HOST }}:{{ env.PORT }}')).toBe('example.com:8080');
        });
        it('should handle env vars in middle of text', () => {
            process.env.NAME = 'World';
            expect((0, index_3.renderEnvOnlyInObject)('Hello {{ env.NAME }}!')).toBe('Hello World!');
        });
        it('should handle templates with newlines', () => {
            process.env.VAR = 'value';
            expect((0, index_3.renderEnvOnlyInObject)(`Line 1: {{ env.VAR }}
Line 2: {{ vars.test }}`)).toBe(`Line 1: value
Line 2: {{ vars.test }}`);
        });
        it('should not confuse env in other contexts', () => {
            process.env.TEST = 'value';
            // Should not match "environment" or other words containing "env"
            expect((0, index_3.renderEnvOnlyInObject)('environment {{ vars.test }}')).toBe('environment {{ vars.test }}');
        });
    });
});
