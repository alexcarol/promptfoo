...
const calculateFilteredMetrics_1 = require("../../src/util/calculateFilteredMetrics");
...
describe('calculateFilteredMetrics', () => {
    ...
    describe('basic metrics aggregation', () => {
        it('should aggregate basic metrics for all results', async () => {
            ...
            const metrics = await (0, calculateFilteredMetrics_1.calculateFilteredMetrics)({
                evalId: eval_.id,
                numPrompts: 1,
        ...
        it('should aggregate metrics across multiple prompts', async () => {
            ...
            for (const promptMetric of metrics) {
                const total = promptMetric.testPassCount + promptMetric.testFailCount + promptMetric.testErrorCount;
                expect(total).toBe(5);
    ...
    describe('filtering with WHERE clause', () => {
        it('should only aggregate results matching WHERE clause', async () => {
            ...
            const whereSql = `eval_id = '${eval_.id}' AND failure_reason = ${index_2.ResultFailureReason.ERROR}`;
            const metrics = await (0, calculateFilteredMetrics_1.calculateFilteredMetrics)({
                evalId: eval_.id,
                numPrompts: 1,
                whereSql,
    ...
    describe('OOM protection', () => {
        it('should throw error when result count exceeds limit', async () => {
            ...
            await expect((0, calculateFilteredMetrics_1.calculateFilteredMetrics)({
                evalId: eval_.id,
                numPrompts: 1,
                whereSql,
                whereParams: [],
...
//# sourceMappingURL=calculateFilteredMetrics.test.js.map