...
const metadata_1 = require("../../../src/codeScan/git/metadata");
...
describe('Git Metadata', () => {
    ...
    describe('extractMetadata', () => {
        it('should extract metadata from git log', async () => {
            ...
            mockGit.revparse.mockImplementation(async (options) => {
                const ref = Array.isArray(options) ? options[0] : options;
                return ref === 'main' ? 'base-sha-123456' : 'compare-sha-789012';
            });
            const metadata = await (0, metadata_1.extractMetadata)('/fake/repo', 'main', 'feature/test');
        ...
        it('should throw GitMetadataError if log extraction fails', async () => {
            ...
            await expect((0, metadata_1.extractMetadata)('/fake/repo', 'main', 'feature/test')).rejects.toThrow(codeScan_1.GitMetadataError);
            await expect((0, metadata_1.extractMetadata)('/fake/repo', 'main', 'feature/test')).rejects.toThrow('Failed to extract git metadata');
        });
