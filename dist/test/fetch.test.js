...
describe('fetchWithRetries', () => {
    ...
    it('should log attempt count with total attempts on rate limit', async () => {
        ...
        await expect((0, index_1.fetchWithRetries)('https://example.com', {}, 1000, 2)).rejects.toThrow();
        expect(logger_1.default.debug).toHaveBeenCalledWith(expect.stringContaining('attempt 1/3'));
    });
    it('should include error details in final error message for rate limits', async () => {
        const rateLimitResponse = (0, utils_1.createMockResponse)({
            status: 429,
            statusText: 'Too Many Requests',
        });
        jest.mocked(global.fetch).mockResolvedValue(rateLimitResponse);
        await expect((0, index_1.fetchWithRetries)('https://example.com', {}, 1000, 2)).rejects.toThrow('Rate limited: 429 Too Many Requests');
    });
});
