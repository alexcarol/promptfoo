...
const worker_1 = require("../../src/python/worker");
...
describeOrSkip('PythonWorker', () => {
    let worker;
    const testScriptPath = path_1.default.join(__dirname, 'fixtures', 'simple_provider.py');
    beforeAll(() => {
        // Create test fixture
...
`);
    ...
    it('should initialize and become ready', async () => {
        worker = new worker_1.PythonWorker(testScriptPath, 'call_api');
    ...
    it('should call different function names dynamically per request', async () => {
        ...
        const multiApiPath = path_1.default.join(__dirname, 'fixtures', 'multi_api_provider.py');
...
`);
        try {
            worker = new worker_1.PythonWorker(multiApiPath, 'call_api');
            await worker.initialize();
    ...
    it('should handle Python errors gracefully', async () => {
        const errorPath = path_1.default.join(__dirname, 'fixtures', 'error_provider.py');
        fs_1.default.writeFileSync(errorPath, `
def call_api(prompt, options, context):
    if prompt == "error":
...
`);
        try {
            worker = new worker_1.PythonWorker(errorPath, 'call_api');
            await worker.initialize();
            // Should succeed
            ...
            if (fs_1.default.existsSync(errorPath)) {
                fs_1.default.unlinkSync(errorPath);
            }
        }
    }, TEST_TIMEOUT);
    it('should handle calling non-existent function gracefully', async () => {
        const nonexistentPath = path_1.default.join(__dirname, 'fixtures', 'test_nonexistent_function.py');
        worker = new worker_1.PythonWorker(nonexistentPath, 'call_api');
        await worker.initialize();
        // Try to call a function that doesn't exist
        // This should throw an error about the function not existing, not ENOENT
        await expect(worker.call('call_nonexistent_api', ['test', {}])).rejects.toThrow(/has no attribute|AttributeError/);
    }, TEST_TIMEOUT);
    it('should provide helpful error message with function name suggestions', async () => {
        const wrongNamePath = path_1.default.join(__dirname, 'fixtures', 'test_wrong_function_name.py');
        worker = new worker_1.PythonWorker(wrongNamePath, 'call_api');
        await worker.initialize();
    ...
    it('should support embeddings-only provider without call_api defined', async () => {
        const embeddingsOnlyPath = path_1.default.join(__dirname, 'fixtures', 'test_embeddings_only.py');
        // Initialize worker with default function name (call_api)
        // This should NOT fail even though call_api doesn't exist
        worker = new worker_1.PythonWorker(embeddingsOnlyPath, 'call_api');
        await worker.initialize();
