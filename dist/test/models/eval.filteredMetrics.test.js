...
describe('Filtered Metrics - WHERE Clause Consistency', () => {
    ...
    describe('CRITICAL: Row count consistency', () => {
        it('should return same row count for pagination and metrics with no filters', async () => {
            ...
            const { testIndices } = await eval_.queryTestIndices({});
            const metrics = await eval_.getFilteredMetrics({});
            const metricsTotal = metrics.reduce((sum, m) => sum + m.testPassCount + m.testFailCount + m.testErrorCount, 0);
        ...
        it('should return same row count for pagination and metrics with filterMode=errors', async () => {
            ...
            const { testIndices } = await eval_.queryTestIndices({ filterMode: 'errors' });
        ...
        it('should return same row count for pagination and metrics with filterMode=failures', async () => {
            ...
            const { testIndices } = await eval_.queryTestIndices({ filterMode: 'failures' });
        ...
        it('should return same row count for pagination and metrics with filterMode=passes', async () => {
            ...
            const { testIndices } = await eval_.queryTestIndices({ filterMode: 'passes' });
        ...
        it('should return same row count for pagination and metrics with searchQuery', async () => {
            ...
            const { testIndices } = await eval_.queryTestIndices({ searchQuery });
        ...
        it('should return same row count for pagination and metrics with metadata filter', async () => {
            ...
            const db = (0, index_1.getDb)();
            await db.run(`UPDATE eval_results SET metadata = json('{"source":"unit"}') WHERE eval_id = '${eval_.id}' AND test_idx IN (1, 3, 5)`);
            const filters = [
                JSON.stringify({
                    logicOperator: 'and',
                    type: 'metadata',
            ...
            const { testIndices } = await eval_.queryTestIndices({ filters });
        ...
        it('should return same row count for pagination and metrics with plugin filter', async () => {
            ...
            const db = (0, index_1.getDb)();
            await db.run(`UPDATE eval_results SET metadata = json('{"pluginId":"harmful:harassment"}') WHERE eval_id = '${eval_.id}' AND test_idx IN (2, 4, 6)`);
            const filters = [
                JSON.stringify({
                    logicOperator: 'and',
                    type: 'plugin',
        ...
        it('should return same row count for pagination and metrics with strategy filter', async () => {
            ...
            const db = (0, index_1.getDb)();
            await db.run(`UPDATE eval_results SET metadata = json('{"strategyId":"jailbreak"}') WHERE eval_id = '${eval_.id}' AND test_idx IN (1, 2, 3)`);
            const filters = [
                JSON.stringify({
                    logicOperator: 'and',
                    type: 'strategy',
        ...
        it('should return same row count for pagination and metrics with severity filter', async () => {
            ...
            const db = (0, index_1.getDb)();
            await db.run(`UPDATE eval_results SET metadata = json('{"severity":"high"}') WHERE eval_id = '${eval_.id}' AND test_idx IN (0, 5)`);
            const filters = [
                JSON.stringify({
                    logicOperator: 'and',
                    type: 'severity',
        ...
        it('should return same row count for pagination and metrics with combined filters', async () => {
            ...
            const db = (0, index_1.getDb)();
            await db.run(`UPDATE eval_results SET metadata = json('{"source":"unit","severity":"high"}') WHERE eval_id = '${eval_.id}' AND test_idx IN (3, 6, 9)`);
            const filters = [
                JSON.stringify({
                    logicOperator: 'and',
                    type: 'metadata',
    ...
    describe('CRITICAL: Metrics correctness', () => {
        it('should calculate correct metrics for filtered failures', async () => {
            ...
            const totalTests = metrics.reduce((sum, m) => sum + m.testPassCount + m.testFailCount + m.testErrorCount, 0);
...
//# sourceMappingURL=eval.filteredMetrics.test.js.map