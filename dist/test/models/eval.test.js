...
describe('evaluator', () => {
    ...
    describe('queryTestIndices', () => {
        ...
        it('filters plugin results with not_equals operator for ids and categories', async () => {
            ...
            const db = (0, index_1.getDb)();
            await db.run(`UPDATE eval_results SET metadata = json('{"pluginId":"harmful:harassment"}') WHERE eval_id = '${eval_.id}' AND test_idx = 0`);
            await db.run(`UPDATE eval_results SET metadata = json('{"pluginId":"bias:toxicity"}') WHERE eval_id = '${eval_.id}' AND test_idx = 1`);
            await db.run(`UPDATE eval_results SET metadata = json('{"pluginId":"custom-plugin"}') WHERE eval_id = '${eval_.id}' AND test_idx = 2`);
            // Leave test_idx = 3 without pluginId to ensure nulls are included
            const excludeSpecificPlugin = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'plugin',
            ...
            const excludeCategory = await eval_.queryTestIndices({
                filters: [
                    JSON.stringify({
                        logicOperator: 'and',
                        type: 'plugin',
