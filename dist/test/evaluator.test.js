...
jest.mock('../src/util/functions/loadFunction', () => ({
    ...
    loadFunction: jest.fn().mockImplementation((options) => {
        if (options.filePath.includes('scoring')) {
            return Promise.resolve((_metrics) => ({
                pass: true,
                score: 0.75,
...
describe('evaluator', () => {
    ...
    it('runs Simba test cases after other providers during evaluate', async () => {
        ...
        const runResults = [
            {
                ...
                provider: {
                    ...
                    label: metadata_1.strategyDisplayNames.simba,
                },
                prompt: {
                    raw: 'Test prompt',
                    label: metadata_1.strategyDisplayNames.simba,
        ...
        await (0, evaluator_1.evaluate)(testSuite, evalRecord, { maxConcurrency: 2 });
        expect(callOrder).toEqual(['normal', 'simba']);
        expect(normalProvider.callApi).toHaveBeenCalledTimes(1);
        expect(agentProvider.runSimba).toHaveBeenCalledTimes(1);
        expect(agentProvider.callApi).not.toHaveBeenCalled();
    ...
    it('forces cache busting for repeat iterations', async () => {
        ...
        const provider = {
            ...
            callApi: jest
                .fn()
                .mockImplementation(async (_prompt, context) => {
...
describe('runEval', () => {
    ...
    it('should include sessionId from response metadata when top-level sessionId is absent', async () => {
        const conversations = {};
        const providerWithMetadataSession = {
        ...
        const [result] = await (0, evaluator_1.runEval)({
            ...defaultOptions,
    ...
    it('delegates Simba providers to runSimba using evaluateOptions maxConcurrency', async () => {
        ...
        expect(runSimbaMock).toHaveBeenCalledWith(expect.objectContaining({
            prompt: 'Simba prompt',
    ...
    it('uses default Simba concurrency when evaluateOptions are not provided', async () => {
        ...
        expect(runSimbaMock).toHaveBeenCalledWith(expect.objectContaining({
            ...
            options: undefined,
            concurrency: constants_1.DEFAULT_MAX_CONCURRENCY,
        }));
