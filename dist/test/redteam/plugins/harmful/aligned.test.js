...
const aligned_1 = require("../../../../src/redteam/plugins/harmful/aligned");
...
describe('AlignedHarmfulPlugin', () => {
    ...
    beforeEach(() => {
        ...
        harmCategory = Object.keys(constants_1.HARM_PLUGINS).find((key) => !(key in constants_1.UNALIGNED_PROVIDER_HARM_PLUGINS));
        const harmfulCategory = {
            key: harmCategory,
        ...
        jest.spyOn(constants_2.REDTEAM_MODEL_CATEGORIES, 'find').mockReturnValue(harmfulCategory);
        plugin = new aligned_1.AlignedHarmfulPlugin(mockProvider, 'test purpose', 'testVar', harmCategory);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('initialization', () => {
        it('should create instance with valid parameters', () => {
            expect(plugin).toBeInstanceOf(aligned_1.AlignedHarmfulPlugin);
            expect(mockProvider.id()).toBe('test-provider');
        });
        it('should accept optional config parameter', () => {
            const configPlugin = new aligned_1.AlignedHarmfulPlugin(mockProvider, 'test purpose', 'testVar', harmCategory, { examples: ['test example'] });
            expect(configPlugin).toBeInstanceOf(aligned_1.AlignedHarmfulPlugin);
        });
    });
    describe('template handling', () => {
        ...
        it('should throw when category config is not found', async () => {
            jest.spyOn(constants_2.REDTEAM_MODEL_CATEGORIES, 'find').mockReturnValue(undefined);
            await expect(plugin['getTemplate']()).rejects.toThrow(`Harmful category ${harmCategory} not found`);
        });
    });
    describe('assertion generation', () => {
        it('should generate harmful assertions for given prompt', () => {
            ...
            const expectedMetric = metadata_1.categoryAliases[harmCategory] || harmCategory;
        ...
        it('should create properly structured test cases', () => {
            ...
            testCases.forEach((testCase) => {
                expect(testCase).toMatchObject({
                    ...
                    metadata: {
                        harmCategory: constants_1.HARM_PLUGINS[harmCategory],
                    },
    ...
    describe('end-to-end test generation', () => {
        ...
        it('should handle custom examples in config', async () => {
            ...
            const configuredPlugin = new aligned_1.AlignedHarmfulPlugin(mockProvider, 'test purpose', 'testVar', harmCategory, { examples: customExamples });
            await configuredPlugin.generateTests(1);
            expect(mockProvider.callApi).toHaveBeenCalledWith(expect.stringContaining('test prompt template Custom example 1,Custom example 2'));
        });
...
//# sourceMappingURL=aligned.test.js.map