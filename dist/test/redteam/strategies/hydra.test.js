...
const hydra_1 = require("../../../src/redteam/strategies/hydra");
describe('addHydra', () => {
    it('should add hydra configuration to test cases', () => {
        ...
        const result = (0, hydra_1.addHydra)(testCases, injectVar, config);
    ...
    it('should handle test cases without assertions', () => {
        ...
        const result = (0, hydra_1.addHydra)(testCases, 'input', {});
        expect(result).toHaveLength(1);
    ...
    it('should handle empty test cases array', () => {
        const result = (0, hydra_1.addHydra)([], 'inject', {});
    ...
    it('should use same scanId for all test cases in a batch', () => {
        ...
        const scanId1 = result[0].provider?.config?.scanId;
        const scanId2 = result[1].provider?.config?.scanId;
        const scanId3 = result[2].provider?.config?.scanId;
    ...
    it('should pass through custom config options', () => {
        ...
        const result = (0, hydra_1.addHydra)(testCases, 'query', customConfig);
        expect(result[0].provider?.config).toMatchObject({
            injectVar: 'query',
            maxTurns: 15,
            maxBacktracks: 5,
            stateful: true,
    ...
    describe('Privacy protection - excludeTargetOutputFromAgenticAttackGeneration', () => {
        it('should pass privacy flag through to provider config when enabled', () => {
            ...
            const result = (0, hydra_1.addHydra)(testCases, 'input', config);
            expect(result).toHaveLength(1);
            expect(result[0].provider?.config?.excludeTargetOutputFromAgenticAttackGeneration).toBe(true);
        });
        it('should pass privacy flag when disabled', () => {
            ...
            const result = (0, hydra_1.addHydra)(testCases, 'input', config);
            expect(result).toHaveLength(1);
            expect(result[0].provider?.config?.excludeTargetOutputFromAgenticAttackGeneration).toBe(false);
        });
        it('should default to undefined when privacy flag not specified', () => {
            ...
            const result = (0, hydra_1.addHydra)(testCases, 'input', config);
            expect(result).toHaveLength(1);
            const privacyFlag = result[0].provider?.config
        ...
        it('should pass privacy flag alongside other config options', () => {
            ...
            const result = (0, hydra_1.addHydra)(testCases, 'query', config);
            expect(result[0].provider?.config).toMatchObject({
                injectVar: 'query',
                maxTurns: 15,
                maxBacktracks: 5,
        ...
        it('should apply privacy flag to all test cases in a batch', () => {
            ...
            result.forEach((testCase) => {
                expect(testCase.provider?.config?.excludeTargetOutputFromAgenticAttackGeneration).toBe(true);
            });
        });
    });
});
