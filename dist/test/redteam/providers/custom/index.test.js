...
const index_1 = require("../../../../src/redteam/providers/custom/index");
...
describe('MemorySystem', () => {
    let memorySystem;
    beforeEach(() => {
        memorySystem = new index_1.MemorySystem();
    });
...
describe('CustomProvider', () => {
    ...
    beforeEach(() => {
        ...
        customProvider = new index_1.CustomProvider({
            injectVar: 'objective',
    ...
    it('should initialize with default config values', () => {
        const provider = new index_1.CustomProvider({
            injectVar: 'objective',
    ...
    it('should include sessionId from target response when stateful is true', async () => {
        ...
        jest.spyOn(provider, 'getAttackPrompt').mockResolvedValue({
            generatedQuestion: 'attack prompt',
        });
        jest.spyOn(provider, 'sendPrompt').mockResolvedValue({
            output: 'target response',
            sessionId: 'response-session-id',
        });
        jest.spyOn(provider, 'getRefusalScore').mockResolvedValue([false, '']);
        jest.spyOn(provider, 'getEvalScore').mockResolvedValue({
            value: false,
            metadata: 0,
            rationale: '',
        });
    ...
    it('should record internal evaluator success without exiting early', async () => {
        ...
        const testProvider = new index_1.CustomProvider({
            injectVar: 'objective',
            strategyText: 'Custom strategy for testing',
    ...
    it('should pass additionalRubric to grader when goal is provided', async () => {
        ...
        jest.mocked(graders_1.getGraderById).mockReturnValue({
            getResult: jest.fn(async (_prompt, _output, _test, _provider, _value, additionalRubric) => {
