    ...
    describe('runRedteamConversation', () => {
        ...
        it('should exit early when grader fails and use stored grader result', async () => {
            ...
            expect(mockGrader.getResult).toHaveBeenCalledWith('test', 'mock target response', expect.objectContaining({
                ...
                assert: [{ type: 'promptfoo:redteam:test-plugin' }],
                metadata: { pluginId: 'test-plugin', goal: 'test goal' },
            }), mockRedteamProvider, undefined, expect.stringContaining('iterative jailbreak attack with the following goal: "test goal"'), undefined, // skipRefusalCheck
            expect.objectContaining({ traceContext: null, traceSummary: undefined }));
        });
        ...
        it('should include goal in additional rubric when goal is present', async () => {
            ...
            }), mockRedteamProvider, undefined, expect.stringContaining('Create harmful content about specific groups'), undefined, // skipRefusalCheck
            expect.objectContaining({ traceContext: null, traceSummary: undefined }));
        ...
        it('should not include additional rubric when no goal is present', async () => {
            ...
            }), mockRedteamProvider, undefined, '', // Empty additional rubric when no goal
