...
describe('Provider Test Functions', () => {
    ...
    beforeEach(() => {
        ...
        index_2.fetchWithProxy.mockReset();
        // Setup mock provider
        mockProvider = {
    ...
    describe('testProviderConnectivity', () => {
        describe('successful connectivity tests', () => {
            it('should successfully test connectivity with agent endpoint analysis', async () => {
                ...
                index_2.fetchWithProxy.mockResolvedValue({
                    ok: true,
                    json: jest.fn().mockResolvedValue(mockAgentResponse),
                });
                const result = await (0, testProvider_1.testProviderConnectivity)(mockProvider);
                ...
                expect(index_2.fetchWithProxy).toHaveBeenCalledWith(expect.stringContaining('/api/v1/providers/test'), expect.objectContaining({
                    method: 'POST',
                    headers: {
            ...
            it('should handle connectivity test with remote grading disabled', async () => {
                ...
                expect(index_2.fetchWithProxy).not.toHaveBeenCalled();
        ...
        describe('failed connectivity tests', () => {
            ...
            it('should handle agent endpoint throwing an exception', async () => {
                ...
                index_2.fetchWithProxy.mockRejectedValue(new Error('Network error'));
                const result = await (0, testProvider_1.testProviderConnectivity)(mockProvider);
                expect(result).toEqual({
                    success: false,
                    message: 'Error evaluating the results. Please review the provider response manually.',
