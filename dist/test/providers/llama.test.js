...
describe('LlamaProvider', () => {
    ...
    describe('constructor', () => {
        it('should initialize with modelName and config', () => {
            const provider = new llama_1.LlamaProvider(modelName, { config });
            expect(provider.modelName).toBe(modelName);
            expect(provider.config).toEqual(config);
        });
        it('should initialize with id function if id is provided', () => {
            const id = 'testId';
            const provider = new llama_1.LlamaProvider(modelName, { config, id });
            expect(provider.id()).toBe(id);
        });
    });
    describe('id', () => {
        it('should return the correct id string', () => {
            const provider = new llama_1.LlamaProvider(modelName);
    ...
    describe('callApi', () => {
        ...
        it('should call fetchWithCache with correct parameters', async () => {
            ...
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith(`${process.env.LLAMA_BASE_URL || 'http://localhost:8080'}/completion`, {
                method: 'POST',
                headers: {
        ...
        it('should return an error if fetchWithCache throws an error', async () => {
            ...
            jest.mocked(cache_1.fetchWithCache).mockRejectedValue(error);
            const provider = new llama_1.LlamaProvider(modelName, { config });
            const result = await provider.callApi(prompt);
        ...
        it('should return an error if response data is malformed', async () => {
            ...
            jest.mocked(cache_1.fetchWithCache).mockResolvedValue(malformedResponse);
            const provider = new llama_1.LlamaProvider(modelName, { config });
            const result = await provider.callApi(prompt);
            expect(result).toEqual({
...
//# sourceMappingURL=llama.test.js.map