...
const transcription_1 = require("../../../src/providers/openai/transcription");
jest.mock('../../../src/cache');
...
describe('OpenAiTranscriptionProvider', () => {
    ...
    beforeEach(() => {
        ...
        jest.mocked(fs_1.default.readFileSync).mockReturnValue(Buffer.from('mock audio data'));
        jest.mocked(cache_1.fetchWithCache).mockResolvedValue(mockTranscriptionResponse);
    });
    describe('Basic functionality', () => {
        it('should transcribe audio successfully', async () => {
            const provider = new transcription_1.OpenAiTranscriptionProvider('gpt-4o-transcribe', {
                config: { apiKey: 'test-key' },
            });
            const result = await provider.callApi('/path/to/audio.mp3');
            expect(fs_1.default.existsSync).toHaveBeenCalledWith('/path/to/audio.mp3');
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith(expect.stringContaining('/audio/transcriptions'), expect.objectContaining({
                method: 'POST',
        ...
        it('should calculate cost correctly for gpt-4o-mini-transcribe', async () => {
            const provider = new transcription_1.OpenAiTranscriptionProvider('gpt-4o-mini-transcribe', {
                config: { apiKey: 'test-key' },
            });
            const result = await provider.callApi('/path/to/audio.mp3');
        ...
        it('should calculate cost correctly for whisper-1', async () => {
            const provider = new transcription_1.OpenAiTranscriptionProvider('whisper-1', {
                config: { apiKey: 'test-key' },
            });
            const result = await provider.callApi('/path/to/audio.mp3');
        ...
        it('should throw an error if API key is not set', async () => {
            ...
            try {
                const provider = new transcription_1.OpenAiTranscriptionProvider('gpt-4o-transcribe');
                await expect(provider.callApi('/path/to/audio.mp3')).rejects.toThrow('OpenAI API key is not set. Set the OPENAI_API_KEY environment variable or add `apiKey` to the provider config.');
            }
            finally {
    ...
    describe('Diarization support', () => {
        it('should handle diarized transcription', async () => {
            const provider = new transcription_1.OpenAiTranscriptionProvider('gpt-4o-transcribe-diarize', {
                config: { apiKey: 'test-key' },
            });
            jest.mocked(cache_1.fetchWithCache).mockResolvedValue(mockDiarizedResponse);
            const result = await provider.callApi('/path/to/audio.mp3');
            expect(result.output).toBe("[0.00s - 2.50s] Speaker 1: Hello, how are you?\n[2.50s - 5.00s] Speaker 2: I'm doing great, thanks!");
    ...
    describe('Error handling', () => {
        ...
        it('should handle API errors', async () => {
            ...
            jest.mocked(cache_1.fetchWithCache).mockResolvedValue(errorResponse);
            const result = await provider.callApi('/path/to/audio.mp3');
        ...
        it('should handle transcription error in catch block', async () => {
            ...
            jest.mocked(cache_1.fetchWithCache).mockImplementation(() => {
    ...
    describe('Configuration options', () => {
        ...
        it('should use custom API URL when provided', async () => {
            ...
            await provider.callApi('/path/to/audio.mp3');
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith(`${customApiUrl}/audio/transcriptions`, expect.any(Object), expect.any(Number), 'json', undefined);
        });
        it('should handle bustCache from context', async () => {
            ...
            await provider.callApi('/path/to/audio.mp3', context);
            expect(cache_1.fetchWithCache).toHaveBeenCalledWith(expect.any(String), expect.any(Object), expect.any(Number), 'json', true);
        });
    ...
    describe('Model validation', () => {
        it('should accept known transcription models', () => {
            ...
            models.forEach((model) => {
                const provider = new transcription_1.OpenAiTranscriptionProvider(model, {
                    config: { apiKey: 'test-key' },
                });
        ...
        it('should allow unknown transcription models with debug log', () => {
            const provider = new transcription_1.OpenAiTranscriptionProvider('unknown-model', {
                config: { apiKey: 'test-key' },
            });
