...
describe('OpenAI Provider', () => {
    ...
    describe('OpenAiChatCompletionProvider', () => {
        ...
        it('should handle OpenAI reasoning field with separate content correctly', async () => {
            const mockResponse = {
                data: {
                    choices: [
                        {
                            message: {
                                reasoning: 'First, I need to analyze the numbers. 9.11 has 11 in the hundredths place, while 9.8 has 8 in the tenths place. Converting 9.8 to hundredths gives 9.80, so 9.11 > 9.80.',
            ...
            mockFetchWithCache.mockResolvedValue(mockResponse);
            const provider = new chat_1.OpenAiChatCompletionProvider('gpt-oss-20b');
            const result = await provider.callApi(JSON.stringify([{ role: 'user', content: 'Which is greater: 9.11 or 9.8?' }]));
            expect(mockFetchWithCache).toHaveBeenCalledTimes(1);
        ...
        it('should hide OpenAI reasoning field when showThinking is false', async () => {
            ...
            const result = await provider.callApi(JSON.stringify([{ role: 'user', content: 'What is the answer?' }]));
            expect(mockFetchWithCache).toHaveBeenCalledTimes(1);
...
Therefore, there are 2 occurrences of the letter "r" in "strawberry".\n\nThere are 2 "r"s in the word "strawberry".`;
        ...
        it('should work with apiKeyRequired: false and API key from environment', () => {
            ...
            try {
                const provider = new chat_1.OpenAiChatCompletionProvider('local-model', {
                    config: {
