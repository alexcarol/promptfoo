...
const workerPool_1 = require("../python/workerPool");
...
class PythonProvider {
    ...
    async initialize() {
        ...
        this.initializationPromise = (async () => {
            try {
                ...
                this.pool = new workerPool_1.PythonWorkerPool(absPath, this.functionName || 'call_api', workerCount, this.config.pythonExecutable, this.config.timeout);
                await this.pool.initialize();
                // Register for cleanup
                providerRegistry_1.providerRegistry.register(this);
                this.isInitialized = true;
                logger_1.default.debug(`Initialized Python provider ${this.id()} with ${workerCount} workers`);
            }
            catch (error) {
    ...
    getWorkerCount() {
        ...
        // 2. Environment variable
            ...
            const envWorkers = (0, pythonUtils_1.getEnvInt)('PROMPTFOO_PYTHON_WORKERS');
        ...
        if (count < 1) {
            logger_1.default.warn(`Invalid worker count ${count}, using minimum of 1`);
            return 1;
        }
        return count;
    }
    ...
    async executePythonScript(prompt, context, apiType) {
        ...
        if (cachedResult) {
            ...
            logger_1.default.debug(`Executing python script ${absPath} via worker pool with args: ${(0, json_1.safeJsonStringify)(args)}`);
            ...
            if (typeof result === 'object' && result !== null && apiType === 'call_api') {
                ...
                if (result.tokenUsage && !result.tokenUsage.numRequests) {
                    ...
                    logger_1.default.debug(`Added numRequests to fresh result token usage: ${JSON.stringify(result.tokenUsage)}`);
                }
            }
            return result;
        }
    ...
    async shutdown() {
        ...
        providerRegistry_1.providerRegistry.unregister(this);
        this.isInitialized = false;
    }
}
