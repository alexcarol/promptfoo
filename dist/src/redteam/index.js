...
const categories = {
    ...
    medical: constants_1.MEDICAL_PLUGINS,
    pharmacy: constants_1.PHARMACY_PLUGINS,
    insurance: constants_1.INSURANCE_PLUGINS,
    financial: constants_1.FINANCIAL_PLUGINS,
};
/**
...
async function applyStrategies(testCases, strategies, injectVar, excludeTargetOutputFromAgenticAttackGeneration) {
    ...
    for (const strategy of strategies) {
        ...
        const languagesInResults = new Set(strategyTestCases
...
async function synthesize({ abortSignal, delay, entities: entitiesOverride, injectVar, language, maxConcurrency = 1, plugins, prompts, provider, purpose: purposeOverride, strategies, targetLabels, showProgressBar: showProgressBarOverride, excludeTargetOutputFromAgenticAttackGeneration, testGenerationInstructions, }) {
    ...
    const hasLanguageDisallowedStrategy = strategies.some((s) => (0, constants_1.isLanguageDisallowedStrategy)(s.id));
    if (hasLanguageDisallowedStrategy && language) {
        const originalLanguage = Array.isArray(language) ? language.join(', ') : language;
        language = 'en';
        logger_1.default.info(`[Language Override] Detected language-disallowed strategy (audio/video/image/layer/math-prompt). Forcing language to 'en' (was: ${originalLanguage})`);
    }
        ...
        if (action) {
            ...
            const languages = Array.isArray(languageConfig)
                ...
                : languageConfig
                    ? [languageConfig]
                    : [undefined];
            logger_1.default.debug(`[Language Processing] Plugin: ${plugin.id}, Languages: ${JSON.stringify(languages)}, NumTests per language: ${plugin.numTests}${plugin.config?.language ? ' (plugin override)' : ''}`);
            ...
            const languagePromises = languages.map(async (lang) => {
                ...
                });
                {
                    ...
                    if (Array.isArray(pluginTests) && pluginTests.length > 0) {
                        ...
                        const testsWithMetadata = pluginTests.map((test) => addLanguageToPluginMetadata(test, lang, plugin, testGenerationInstructions));
                        return {
                            lang: langKey,
                    ...
                    logger_1.default.warn(`[Language Processing] No tests generated for ${plugin.id} in language: ${lang || 'default'}`);
                    return {
                        lang: langKey,
                        tests: [],
                        requested: plugin.numTests,
            ...
            for (const result of languageResults) {
                if (result.status === 'fulfilled') {
                    ...
                    logger_1.default.warn(`[Language Processing] Error generating tests for ${plugin.id}: ${result.reason}`);
                }
            }
            logger_1.default.debug(`[Language Processing] Total tests generated for ${plugin.id}: ${allPluginTests.length} (across ${languages.length} language(s))`);
            if (!Array.isArray(allPluginTests) || allPluginTests.length === 0) {
                ...
                for (const testCase of testCasesWithMetadata) {
                    ...
                    const extractedGoal = await (0, util_2.extractGoalFromPrompt)(prompt, purpose, plugin.id, policy);
            ...
            if (showProgressBar) {
                ...
                logger_1.default.info(`Generated ${allPluginTests.length} tests for ${plugin.id}`);
            }
            logger_1.default.debug(`Added ${allPluginTests.length} ${plugin.id} test cases`);
    ...
    checkAbort();
    const { testCases: strategyTestCases, strategyResults: otherStrategyResults } = await applyStrategies(pluginTestCases, strategies.filter((s) => !['basic', 'retry'].includes(s.id)), injectVar, excludeTargetOutputFromAgenticAttackGeneration);
