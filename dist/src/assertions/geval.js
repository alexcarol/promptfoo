...
const handleGEval = async ({ assertion, renderedValue, prompt, outputString, test, providerCallContext, }) => {
    ...
    if (Array.isArray(renderedValue)) {
        ...
        for (const value of renderedValue) {
            const resp = await (0, matchers_1.matchesGEval)(value, prompt || '', outputString, threshold, test.options, providerCallContext);
        ...
        const resp = await (0, matchers_1.matchesGEval)(renderedValue, prompt || '', outputString, threshold, test.options, providerCallContext);
        return {
            assertion,
            ...resp,
        };
