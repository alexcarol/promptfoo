...
exports.validateLinkedTargetId = validateLinkedTargetId;
...
function makeRequest(path, method, body) {
    ...
    try {
        return (0, index_1.fetchWithProxy)(url, {
            method,
...
async function validateLinkedTargetId(linkedTargetId) {
    ...
    if (!isCloudProvider(linkedTargetId)) {
        ...
        linkedTargetId must start with "${constants_1.CLOUD_PROVIDER_PREFIX}" followed by a target ID.
        Example: ${constants_1.CLOUD_PROVIDER_PREFIX}12345678-1234-1234-1234-123456789abc
        ...
        4. Format as: ${constants_1.CLOUD_PROVIDER_PREFIX}<target-id>
      `);
    }
    // Check existence in cloud (if enabled)
    if (!cloud_1.cloudConfig.isEnabled()) {
        logger_1.default.warn('[Cloud] linkedTargetId specified but cloud is not configured', {
            linkedTargetId,
    ...
    try {
        logger_1.default.debug('[Cloud] Validating linkedTargetId exists in cloud', {
            linkedTargetId,
            providerId,
        });
        await getProviderFromCloud(providerId);
        logger_1.default.debug('[Cloud] linkedTargetId validation successful', {
            linkedTargetId,
        });
    }
    catch (error) {
        logger_1.default.error('[Cloud] linkedTargetId validation failed', {
            linkedTargetId,
            error,
        });
        const apiHost = cloud_1.cloudConfig.getApiHost();
...
async function getOrgContext() {
    ...
    try {
        ...
        const currentTeamId = cloud_1.cloudConfig.getCurrentTeamId(organization.id);
