...
exports.calculateFilteredMetrics = calculateFilteredMetrics;
...
async function calculateFilteredMetrics(opts) {
    ...
    try {
        ...
        if (countResult > MAX_RESULTS_FOR_METRICS) {
            logger_1.default.warn(`Filtered result count ${countResult} exceeds limit ${MAX_RESULTS_FOR_METRICS}`, {
                evalId: opts.evalId,
            });
        ...
        logger_1.default.error('Failed to calculate filtered metrics with optimized query', { error });
 ...
 */
async function getResultCount(whereSql, _params) {
    const db = (0, index_1.getDb)();
    const query = drizzle_orm_1.sql.raw(`
...
async function calculateWithOptimizedQuery(opts) {
    ...
    const basicMetricsQuery = drizzle_orm_1.sql.raw(`
    SELECT
      prompt_idx,
      COUNT(DISTINCT test_idx) as total_count,
      SUM(CASE WHEN success = 1 THEN 1 ELSE 0 END) as pass_count,
      SUM(CASE WHEN success = 0 AND failure_reason != ${index_2.ResultFailureReason.ERROR} THEN 1 ELSE 0 END) as fail_count,
      SUM(CASE WHEN failure_reason = ${index_2.ResultFailureReason.ERROR} THEN 1 ELSE 0 END) as error_count,
  ...
  `);
    ...
    for (const row of basicResults) {
        ...
        if (idx < 0 || idx >= numPrompts) {
            logger_1.default.warn(`Invalid prompt_idx ${idx}, expected 0-${numPrompts - 1}`);
            continue;
        }
        metrics[idx] = {
    ...
    logger_1.default.debug('Filtered metrics calculated', {
        numPrompts,
 ...
 */
async function aggregateNamedScores(metrics, whereSql, _params) {
    const db = (0, index_1.getDb)();
 ...
 */
async function aggregateAssertions(metrics, whereSql, _params) {
    const db = (0, index_1.getDb)();
...
//# sourceMappingURL=calculateFilteredMetrics.js.map