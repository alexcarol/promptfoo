...
const TracingConfigSchema = zod_1.z.lazy(() => zod_1.z.object({
    enabled: zod_1.z.boolean().optional(),
    includeInAttack: zod_1.z.boolean().optional(),
    includeInGrading: zod_1.z.boolean().optional(),
    includeInternalSpans: zod_1.z.boolean().optional(),
    maxSpans: zod_1.z.number().int().positive().optional(),
    maxDepth: zod_1.z.number().int().positive().optional(),
    maxRetries: zod_1.z.number().int().nonnegative().optional(),
    retryDelayMs: zod_1.z.number().int().nonnegative().optional(),
    spanFilter: zod_1.z.array(zod_1.z.string()).optional(),
    sanitizeAttributes: zod_1.z.boolean().optional(),
    strategies: zod_1.z.record(zod_1.z.lazy(() => TracingConfigSchema)).optional(),
}));
const frameworkOptions = constants_1.FRAMEWORK_COMPLIANCE_IDS;
...
exports.RedteamGenerateOptionsSchema = zod_1.z.object({
    ...
    frameworks: zod_1.z
        .array(zod_1.z.enum(frameworkOptions))
        .min(1)
        .optional()
        .describe('Subset of compliance frameworks to include when generating, reporting, and filtering results'),
...
exports.RedteamConfigSchema = zod_1.z
    ...
    const handleCollectionExpansion = (id, config, numTests, severity) => {
        if (id === 'foundation') {
            ...
            expandCollection([...constants_1.PHARMACY_PLUGINS], config, numTests, severity);
        }
        else if (id === 'insurance') {
            expandCollection([...constants_1.INSURANCE_PLUGINS], config, numTests, severity);
        }
        else if (id === 'financial') {
            expandCollection([...constants_1.FINANCIAL_PLUGINS], config, numTests, severity);
        }
        else if (id === 'default') {
